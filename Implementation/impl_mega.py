...
def transform_sma_data(xtrain_dfs, xtest_dfs, ytrain_dfs, ytest_dfs, do_preprocess=True):
    x_train = pd.concat(xtrain_dfs.values(), axis=0)
    y_train = pd.concat(ytrain_dfs.values(), axis=0)

    x_test = pd.concat(xtest_dfs.values(), axis=0)
    y_test = pd.concat(ytest_dfs.values(), axis=0)

    train_data = x_train.to_numpy()
    train_labels = y_train.to_numpy().reshape(-1)

    test_data = x_test.to_numpy()
    test_labels = y_test.to_numpy().reshape(-1)
...
if __name__ == '__main__':
    ...
    (x_train, _), (x_test, y_test) = transform_sma_data(xtrain_dfs, xtest_dfs, ytrain_dfs, ytest_dfs)
    ...
    # Define Model
    model = MultWaveGCUNet(input_channel=137, embedding_dim=args.embedding_dim, top_k=args.top_k,
